...
export function EditPairModal({ isOpen, onClose, onEdit, initialPair }: EditPairModalProps) {
  ...
  useEffect(() => {
    if (initialPair && isOpen) {
      setPlayer1(initialPair.player1 || '');
      setPlayer2(initialPair.player2 || '');
      setPaymentStatus(initialPair.paymentStatus || (initialPair.isPaid ? 'paid' : 'unpaid'));
      setDepositAmount(initialPair.depositAmount && initialPair.depositAmount !== 'full' && initialPair.depositAmount !== '0' ? initialPair.depositAmount : '');
    }
  }, [initialPair, isOpen]);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-6">
          ...
          >
            <div className="flex items-center justify-between p-4 lg:p-6 border-b border-outline-variant/60 bg-surface-container/50">
              <h2 className="font-headline font-extrabold text-xl text-on-surface">Editar Pareja</h2>
              <button 
                onClick={onClose}
