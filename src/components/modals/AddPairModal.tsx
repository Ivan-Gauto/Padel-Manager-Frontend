...
import { X, UserPlus, Search, DollarSign } from 'lucide-react';
...
export function AddPairModal({ isOpen, onClose, onAdd }: AddPairModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (player1 && player2) {
      onAdd({
        ...
        isPaid: paymentStatus === 'paid' // Keeping for backwards compatibility if needed
      });
      setPlayer1('');
      setPlayer2('');
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-6">
          ...
          >
            <div className="flex items-center justify-between p-4 lg:p-6 border-b border-outline-variant/60 bg-surface-container/50">
              <h2 className="font-headline font-extrabold text-xl text-on-surface">Agregar Nueva Pareja</h2>
              <button 
                onClick={onClose}
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2 flex items-center gap-2">
                    <Search size={14} className="text-primary" /> Jugador 1
                  </label>
                ...
                <div>
                  <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2 flex items-center gap-2">
                    <Search size={14} className="text-primary" /> Jugador 2
                  </label>
                  <div className="relative">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/40" size={18} />
                    <input 
                ...
                <div>
                  <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2 flex items-center gap-2">
                    <DollarSign size={14} className="text-tertiary" /> Estado de Pago
                  </label>
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <button
                      ...
                      className={`py-3 px-2 rounded-xl border text-xs font-bold transition-all ${paymentStatus === 'paid' ? 'bg-primary/10 border-primary/50 text-primary' : 'border-outline/50 text-on-surface-variant hover:bg-surface/50'}`}
                    >
                      Pagado
                    </button>
                  </div>
